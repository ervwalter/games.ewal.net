    ...
    internal class BggClient
    {
        ...
        private async Task<XDocument> DownloadDataAsync(string url)
        {
            ...
            try
            {
                ...
                while (data == null && retries < 5)
                {
                    ...
                    {
                        _log.LogInformation("DEBUG: HttpRequestException: {Message}", ex.Message);
                        if (ex.StatusCode.HasValue)
                        {
                            _log.LogInformation("DEBUG: StatusCode: {StatusCode}", ex.StatusCode.Value);
                        }
                        throw;
                    }
                    catch (Exception ex)
        ...
        public async Task LoginAsync(string username, string password)
        {
            ...
            try
            {
                ...
                var payload = JsonConvert.SerializeObject(new { credentials = new { username, password } });
                ...
                var usernameCookie = _httpClientHandler.CookieContainer.GetCookies(new Uri("https://boardgamegeek.com"))["bggusername"];
