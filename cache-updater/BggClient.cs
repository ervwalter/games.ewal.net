            ...
            try
            {
                ...
                try
                {
                    try
                    {
                        ...
                        _log.LogInformation("Login response status: {StatusCode}", response.StatusCode);
                        if (!response.IsSuccessStatusCode)
                        {
                            var responseBody = await response.Content.ReadAsStringAsync();
                            _log.LogError("Login failed with status {StatusCode}: {ResponseBody}", response.StatusCode, responseBody);
                            throw new Exception($"Login request failed with status {response.StatusCode}");
                        }

                        // BGG sends deletion headers for old cookies that conflict with new HttpOnly cookies in CookieContainer
                        var cookies = _cookies.GetCookies(new Uri("https://boardgamegeek.com"));
                        if (cookies["bggusername"] == null && response.Headers.TryGetValues("Set-Cookie", out var headers))
                        {
                            foreach (var h in headers.Where(h => h.Contains("HttpOnly")))
                            {
                                var parts = h.Split(';')[0].Split('=', 2);
                                if (parts.Length == 2)
                                    _cookies.Add(new Uri("https://boardgamegeek.com"),
                                        new Cookie(parts[0].Trim(), parts[1].Trim()) { Domain = "boardgamegeek.com", Path = "/" });
                            }
                        }
                    }
                    catch (HttpRequestException ex)
                    {
                        _log.LogError("HTTP request exception during login: {Message}", ex.Message);
                    ...
                    var usernameCookie = allCookies["bggusername"];
                    var passwordCookie = allCookies["bggpassword"];

                    if (usernameCookie == null || string.IsNullOrWhiteSpace(usernameCookie.Value))
                    {
                        _log.LogError("Login failed: 'bggusername' cookie not set");
                        throw new Exception("Login failed: 'bggusername' cookie not set. The login endpoint may have changed.");
                    }

                    if (passwordCookie == null || string.IsNullOrWhiteSpace(passwordCookie.Value))
                    {
                        _log.LogWarning("Login succeeded but 'bggpassword' cookie not set - may cause issues with private data");
                    }

                    _log.LogInformation("Login successful for user '{Username}'", username);
                }
                finally
                {
                    response?.Dispose();
